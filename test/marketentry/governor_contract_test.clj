  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Bairiki Fabrication Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Bairiki Fabrication Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest procurement-tier-understated-is-held-and-unoverridable
  (testing "FLAGSHIP: declared LVP but AUD 4,000/month x 18 months = AUD 72,000 accumulated -> HVP -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (some #{:procurement-tier-understated} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest reserved-activity-violation-is-held-and-unoverridable
  (testing "foreign investor declaring a Schedule 1 Reserved activity (taxi transport) -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:reserved-activity-violation} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
...
(deftest tin-record-unverified-is-held-and-unoverridable
  (testing "unverified Kiribati Tax Division TIN record when required -> HARD hold"
    (let [[db actor] (fresh)
