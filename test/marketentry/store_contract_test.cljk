...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Ltd" :jurisdiction "KIR"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0
                                   :declared-procurement-tier :mvp :declared-procurement-value-aud 12000.0
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "KIR" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:declared-procurement-value-aud (:engagement m)) (:declared-procurement-value-aud (:engagement d))))
    (is (= (:declared-procurement-tier (:engagement m)) (:declared-procurement-tier (:engagement d))))
